          ...
          CreateView =
            fun (widget, treeContext, parentNode) ->
                ...
                let struct (node, view) =
                    memoizedDef.CreateView(memoizedWidget, treeContext, parentNode)

                // store widget that was used to produce this node
                // to pass it to reconciler later on
                node.MemoizedWidget <- Some memoizedWidget
                struct (node, view)
          AttachView = fun (_widget, _treeContext, _parentNode, _view) -> failwith "Memo widget cannot be attached" }
