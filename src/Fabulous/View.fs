...
module ViewHelpers =
    let canReuseView (prevWidget: inref<Widget>) (currWidget: inref<Widget>) =
...
module View =
    ...
    let lazy'<'msg, 'key, 'marker when 'msg: equality and 'key: equality>
        ...
        let att = Memo.MemoAttribute.WithValue(memo)
        WidgetBuilder<'msg, Memo.Memoized<'marker>>(Memo.MemoWidgetKey, &att)
